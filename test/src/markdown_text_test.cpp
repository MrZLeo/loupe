...
TEST_CASE("parse markdown inline spans", "[markdown_text]") {
  const auto blocks = agentlens::parse_markdown_text(
      "hello **bold** and *em* plus `code` [link](https://example.test)");

  REQUIRE(blocks.size() == 1);
  REQUIRE(blocks[0].kind == agentlens::MarkdownBlockKind::Paragraph);
...
TEST_CASE("parse markdown block types", "[markdown_text]") {
  ...
  REQUIRE(blocks[0].kind == agentlens::MarkdownBlockKind::Heading);
  REQUIRE(blocks[0].level == 1);
  REQUIRE(blocks[0].spans[1].bold);

  REQUIRE(blocks[1].kind == agentlens::MarkdownBlockKind::ListItem);
  REQUIRE(blocks[1].marker == "-");
  REQUIRE(blocks[1].spans[1].code);

  REQUIRE(blocks[2].kind == agentlens::MarkdownBlockKind::ListItem);
  REQUIRE(blocks[2].marker == "1.");

  REQUIRE(blocks[3].kind == agentlens::MarkdownBlockKind::Quote);
...
TEST_CASE("parse fenced markdown code block", "[markdown_text]") {
  ...
  REQUIRE(blocks[0].kind == agentlens::MarkdownBlockKind::CodeBlock);
  REQUIRE(blocks[0].code == "{\"a\":1}");
  REQUIRE(blocks[1].kind == agentlens::MarkdownBlockKind::Paragraph);
  REQUIRE(blocks[1].spans[0].text == "after");
}

TEST_CASE("keep plain lines as separate paragraphs", "[markdown_text]") {
  const auto blocks = agentlens::parse_markdown_text("one\ntwo");
